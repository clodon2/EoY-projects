...
while running1:
    ...
    while running2:
        ...
        theyInput = theyInput.replace(" ", "")
# ^ gets rid of any spaces in the response, since they register as a non letter
        if theyInput.isalpha():
            running2 = False
        elif theyInput == "":
            print("At least enter something first!")
            print("")
        else:
...
# ^ checks that the input given contains only letters, since anything else causes errors
    ...
    ULetters = Fcount[0]
    LLetters = Fcount[1]
# ^ uses the case counter aka "ULcount" function and separates it into the number of uppercase and lowercase letters
